...
fn run_python_export() -> Option<Value> {
  ...
  let output = std::process::Command::new("conda")
    .args(["run", "-n", "trade", "python3", "tests/talib_validation.py", "--export-json"])
    .output()
    .or_else(|_| {
  ...
  if !output.status.success() {
    ...
    eprintln!("[talib_crossval] Python script failed (exit {}):", output.status);
    eprintln!("{stderr}");
    return None;
  }
  ...
  match serde_json::from_str::<Value>(&stdout) {
    ...
    Err(e) => {
      ...
      eprintln!("[talib_crossval] First 500 chars: {}", &stdout[..stdout.len().min(500)]);
      None
    },
  }
}
...
fn run_python_enhanced_export() -> Option<Value> {
  let output = std::process::Command::new("conda")
    .args(["run", "-n", "trade", "python3", "tests/talib_validation.py", "--export-enhanced-json"])
    .output()
    .or_else(|_| {
      std::process::Command::new("python3")
  ...
  if !output.status.success() {
    let stderr = String::from_utf8_lossy(&output.stderr);
    eprintln!("[talib_crossval] Enhanced Python script failed (exit {}):", output.status);
    eprintln!("{stderr}");
    return None;
  }
...
fn talib_lookback(talib_name: &str) -> usize {
  ...
  let extra = match talib_name {
    // 1-bar patterns
    "CDLDOJI" | "CDLDRAGONFLYDOJI" | "CDLGRAVESTONEDOJI" | "CDLLONGLEGGEDDOJI" | "CDLHIGHWAVE"
    | "CDLSPINNINGTOP" | "CDLLONGLINE" | "CDLSHORTLINE" | "CDLMARUBOZU" | "CDLCLOSINGMARUBOZU"
    | "CDLBELTHOLD" | "CDLRICKSHAWMAN" | "CDLTAKURI" => 0,
...
fn compare_pattern(
  ...
  for (i, talib_val) in talib_results.iter().enumerate() {
    ...
    let yacpd_match =
      grouped.get(i).and_then(|pats| pats.iter().find(|p| p.pattern_id.0 == yacpd_id));
...
fn talib_crossval_all_61_patterns() {
  ...
  assert!(test_cases.is_object(), "Expected 'test_cases' object in JSON");
  ...
  if !all_failures.is_empty() {
    let mut msg = format!("{} of {total_patterns} patterns had mismatches:\n", all_failures.len());
...
fn talib_crossval_fuzz() {
  ...
  for round in fuzz {
    ...
    let bars_json = round["bars"].as_array().expect("missing 'bars' in fuzz round");
    let bars = json_bars_to_test_bars(bars_json);
  ...
  if !all_failures.is_empty() {
    ...
    for (seed, name, mismatches) in &all_failures[..show] {
      msg.push_str(&format!("  seed={seed} {name}: {} mismatches\n", mismatches.len()));
...
fn talib_crossval_enhanced_fuzz() {
  ...
  for round in fuzz {
    ...
    let bars_json = round["bars"].as_array().expect("missing 'bars' in enhanced fuzz round");
    let bars = json_bars_to_test_bars(bars_json);

    let results = round["results"].as_object().expect("missing 'results' in enhanced fuzz round");
...
fn talib_crossval_edge_cases() {
  ...
  for (case_name, case_data) in edge_cases {
    ...
    let bars_json = case_data["bars"].as_array().expect("missing 'bars' in edge case");
    let bars = json_bars_to_test_bars(bars_json);
    let results = case_data["results"].as_object().expect("missing 'results' in edge case");
  ...
  if !all_failures.is_empty() {
    ...
    for (case_name, name, mismatches) in &all_failures[..show] {
      msg.push_str(&format!("  {case_name}/{name}: {} mismatches\n", mismatches.len()));
      for m in mismatches.iter().take(3) {
        msg.push_str(&format!("    {m}\n"));
      }
    }
...
fn talib_crossval_real_data() {
  ...
  for (ticker, ticker_data) in real_data {
    ...
    let bars_json = ticker_data["bars"].as_array().expect("missing 'bars' in real data");
    let bars = json_bars_to_test_bars(bars_json);
    let bar_count = bars.len();
    eprintln!("[{ticker}] Testing {bar_count} bars...");

    let results = ticker_data["results"].as_object().expect("missing 'results' in real data");
  ...
  if total_mismatches > max_allowed_mismatches {
    ...
    for (ticker, name, mismatches) in &all_failures[..show] {
      msg.push_str(&format!("  {ticker}/{name}: {} mismatches\n", mismatches.len()));
      for m in mismatches.iter().take(3) {
        msg.push_str(&format!("    {m}\n"));
      }
    }
