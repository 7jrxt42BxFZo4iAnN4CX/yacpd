...
fn test_dragonfly_doji_detection() {
  ...
  let engine = EngineBuilder::new()
    .add(BuiltinDetector::DragonflyDoji(DragonflyDojiDetector::with_defaults()))
    .build()
    .unwrap();
...
fn test_gravestone_doji_detection() {
  ...
  let engine = EngineBuilder::new()
    .add(BuiltinDetector::GravestoneDoji(GravestoneDojiDetector::with_defaults()))
    .build()
    .unwrap();
...
#[test]
fn test_two_bars() {
  let bars = vec![TestBar::new(100.0, 105.0, 95.0, 102.0), TestBar::new(102.0, 107.0, 97.0, 104.0)];
...
fn test_no_false_positives_on_flat_data() {
  ...
  let bars: Vec<TestBar> = (0..100).map(|_| TestBar::new(100.0, 100.5, 99.5, 100.1)).collect();

  let engine = EngineBuilder::new()
    .add(BuiltinDetector::Hammer(HammerDetector::with_defaults()))
    .add(BuiltinDetector::MorningStar(MorningStarDetector::with_defaults()))
    .build()
    .unwrap();

  let patterns = engine.scan(&bars).unwrap();
  assert!(patterns.is_empty(), "Should not detect patterns on flat data");
}
...
fn test_custom_detector() {
  ...
  let engine = EngineBuilder::new().add_custom(CustomDetector).build().unwrap();
