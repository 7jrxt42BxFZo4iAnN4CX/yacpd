...
impl Default for EngulfingDetector {
  fn default() -> Self {
    Self { min_engulf_ratio: Ratio::new_const(1.0) }
  }
}
...
impl Default for CounterattackDetector {
  fn default() -> Self {
    Self { close_tolerance: Ratio::new_const(0.01) }
  }
}
...
impl PatternDetector for CounterattackDetector {
  ...
  fn detect<T: OHLCV>(
    ...
    let equal_threshold = helpers::trailing_avg_range(bars, index - 1, 5) * helpers::EQUAL_FACTOR;
...
impl Default for InNeckDetector {
  fn default() -> Self {
    Self { tolerance: Ratio::new_const(0.03) }
  }
}
...
impl PatternDetector for KickingByLengthDetector {
  ...
  fn detect<T: OHLCV>(
    ...
    // Gap required
    let has_gap =
      if curr.is_bullish() { curr.low() > prev.high() } else { curr.high() < prev.low() };
    if !has_gap {
      return None;
    }
...
impl PatternDetector for SeparatingLinesDetector {
  ...
  fn detect<T: OHLCV>(
    ...
    let equal_threshold = helpers::trailing_avg_range(bars, index - 1, 5) * helpers::EQUAL_FACTOR;
    if curr.open() > prev.open() + equal_threshold || curr.open() < prev.open() - equal_threshold {
      return None;
    }
    ...
    let direction = if curr_white { Direction::Bullish } else { Direction::Bearish };
...
impl PatternDetector for GapSideSideWhiteDetector {
  ...
  fn detect<T: OHLCV>(
    ...
    let direction = if gap_up { Direction::Bullish } else { Direction::Bearish };
...
impl ParameterizedDetector for EngulfingDetector {
  ...
  fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
    Ok(Self { min_engulf_ratio: get_ratio(params, "min_engulf_ratio", 1.0)? })
  }
...
impl ParameterizedDetector for HaramiDetector {
  ...
  fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
    Ok(Self { max_body_ratio: get_ratio(params, "max_body_ratio", 0.5)? })
  }
...
impl ParameterizedDetector for HaramiCrossDetector {
  ...
  fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
    Ok(Self { doji_body_max_ratio: get_ratio(params, "doji_body_max_ratio", 0.1)? })
  }
...
impl ParameterizedDetector for PiercingDetector {
  ...
  fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
    Ok(Self { min_pierce_ratio: get_ratio(params, "min_pierce_ratio", 0.5)? })
  }
...
impl ParameterizedDetector for CounterattackDetector {
  ...
  fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
    Ok(Self { close_tolerance: get_ratio(params, "close_tolerance", 0.01)? })
  }
...
impl ParameterizedDetector for KickingDetector {
  ...
  fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
    Ok(Self { shadow_max_ratio: get_ratio(params, "shadow_max_ratio", 0.05)? })
  }
