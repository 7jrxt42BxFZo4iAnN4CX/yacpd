...
impl PatternDetector for BreakawayDetector {
  ...
  fn detect<T: OHLCV>(
    ...
    let color_second = if second.close() >= second.open() { 1 } else { -1 };
    let color_fourth = if fourth.close() >= fourth.open() { 1 } else { -1 };
...
impl PatternDetector for ConcealingBabySwallowDetector {
  ...
  fn detect<T: OHLCV>(
    ...
    if !first.is_bearish() || !second.is_bearish() || !third.is_bearish() || !fourth.is_bearish() {
      return None;
    }
