...
use super::helpers::{
    self, is_body_long_f, is_body_short_f, is_doji_f, is_shadow_very_short, is_shadow_very_short_f,
};
use crate::{
...
impl PatternDetector for ThreeWhiteSoldiersDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        let svs_first =
            helpers::trailing_avg_range(bars, index - 2, 10) * self.shadow_veryshort_factor;
        let svs_second =
...
impl PatternDetector for ThreeLineStrikeDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        let near_threshold_first =
            helpers::trailing_avg_range(bars, index - 3, 5) * self.near_factor;
        let near_threshold_second =
            helpers::trailing_avg_range(bars, index - 2, 5) * self.near_factor;
...
impl PatternDetector for StickSandwichDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        let equal_threshold =
            helpers::trailing_avg_range(bars, index - 2, 5) * helpers::EQUAL_FACTOR;
    ...
    fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
        Ok(Self {
            ...
            near_factor: params
                .get("near_factor")
                .copied()
                .unwrap_or(helpers::NEAR_FACTOR),
            far_factor: params
                .get("far_factor")
                .copied()
                .unwrap_or(helpers::FAR_FACTOR),
