...
impl PatternDetector for PriceLinesDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        let all_bullish =
            (start..=index).all(|i| bars.get(i).map(|b| b.is_bullish()).unwrap_or(false));
        ...
        let all_bearish =
            (start..=index).all(|i| bars.get(i).map(|b| b.is_bearish()).unwrap_or(false));
...
impl PatternDetector for FallingWindowDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        if curr.high() < prev.low() {
            ...
            let strength = if avg_range > 0.0 {
                (gap_size / avg_range).min(1.0) * 0.5 + 0.5
            } else {
                0.7
            };
...
impl Default for AboveTheStomachDetector {
    fn default() -> Self {
        Self {
            penetration: Ratio::new_const(0.0),
        }
    }
}
...
impl PatternDetector for AboveTheStomachDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        if curr.open() >= penetration_level {
            let strength =
                0.6 + (curr.open() - prev_midpoint) / prev.body().max(f64::EPSILON) * 0.2;
            return Some(PatternMatch {
                pattern_id: PatternDetector::id(self),
                direction: Direction::Bullish,
                strength: strength.clamp(0.5, 1.0),
...
impl PatternDetector for BelowTheStomachDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        if curr.open() <= penetration_level {
            let strength =
                0.6 + (prev_midpoint - curr.open()) / prev.body().max(f64::EPSILON) * 0.2;
            return Some(PatternMatch {
                pattern_id: PatternDetector::id(self),
                direction: Direction::Bearish,
                strength: strength.clamp(0.5, 1.0),
...
impl Default for DeliberationDetector {
    fn default() -> Self {
        Self {
            body_pct: Ratio::new_const(0.3),
            long_body_pct: Ratio::new_const(0.6),
        }
    }
}
...
impl PatternDetector for TwoBlackGappingDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        let avg_body = (first.body() + second.body()) / 2.0;
        let strength = if avg_body > 0.0 {
            0.6 + (gap_size / avg_body).min(0.4)
        } else {
            0.7
        };
...
impl Default for BlackMarubozuDetector {
    fn default() -> Self {
        Self {
            shadow_tolerance: Ratio::new_const(0.01),
        }
    }
}
...
impl PatternDetector for ShortBlackDetector {
    ...
    fn detect<T: OHLCV>(
        ...
        let reference = if ctx.volatility > 0.0 {
            ctx.volatility
        } else {
            bar.range()
        };
...
impl Default for BlackSpinningTopDetector {
    fn default() -> Self {
        Self {
            body_pct: Ratio::new_const(0.3),
            shadow_ratio: Ratio::new_const(0.5),
        }
    }
}
...
impl Default for ShootingStar2LinesDetector {
    fn default() -> Self {
        Self {
            body_pct: Ratio::new_const(0.3),
            shadow_ratio: Ratio::new_const(2.0),
        }
    }
}
...
impl Default for DownsideTasukiGapDetector {
    fn default() -> Self {
        Self {
            gap_fill_pct: Ratio::new_const(0.7),
        }
    }
}
...
impl ParameterizedDetector for AboveTheStomachDetector {
    ...
    fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
        Ok(Self {
            penetration: get_ratio(params, "penetration", 0.0)?,
        })
    }
...
impl ParameterizedDetector for LastEngulfingBottomDetector {
    ...
    fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
        Ok(Self {
            trend_period: get_period(params, "trend_period", 14)?,
        })
    }
...
impl ParameterizedDetector for BlackMarubozuDetector {
    ...
    fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
        Ok(Self {
            shadow_tolerance: get_ratio(params, "shadow_tolerance", 0.01)?,
        })
    }
...
impl ParameterizedDetector for LongBlackDayDetector {
    ...
    fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
        Ok(Self {
            body_pct: get_ratio(params, "body_pct", 0.7)?,
        })
    }
...
impl ParameterizedDetector for DownsideTasukiGapDetector {
    ...
    fn with_params(params: &HashMap<&str, f64>) -> Result<Self> {
        Ok(Self {
            gap_fill_pct: get_ratio(params, "gap_fill_pct", 0.7)?,
        })
    }
